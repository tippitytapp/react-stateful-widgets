...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.filter(pgm => featId === pgm.id)[0].name
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: featId ? "gold" : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setFeatId(dev.id)}}>Feature</button>
            </div>
          )
        }
      </div>
